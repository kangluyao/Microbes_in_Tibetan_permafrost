...
bin_infor <- read.csv(bin_infor_file) %>%
  dplyr::select(c("genome", "completeness", "contamination", "size")) %>%
...
metabolic_tab <- read.csv(metabolic_output_file) %>%
  dplyr::select(c('Category', 'Function', grep('Function.presence', colnames(.))))
```
...
tax_tab %>% dplyr::select(c("Class")) %>%
  ...
  filter(!Class %in% "") %>% #remove one row with the class of empty
...
```{r}
dat2 <- abundance_tab %>% 
  mutate(SUR = rowMeans(dplyr::select(., grep('SUR', colnames(abundance_tab), value = T)))) %>%
  mutate(SUB = rowMeans(dplyr::select(., grep('SUB', colnames(abundance_tab), value = T)))) %>%
  mutate(PL = rowMeans(dplyr::select(., grep('PL', colnames(abundance_tab), value = T)))) %>%
  dplyr::select(c(ID, SUR, SUB, PL)) %>%
...
num.mags.df <- dat3 %>% dplyr::select(c(1, 2, 4)) %>%
  group_by(Category, Function) %>% 
  summarise(across(everything(), sum)) %>%
  filter(Function %in% c(C_N_metapathway, other_metapathway)) %>% 
...
dat_3layers <- metabolic_tab %>% 
  ...
  dplyr::select(c('Category', 'Function', 'ratio_sur', 'ratio_sub', 'ratio_pl'))
...
Input files
```{r}
sur_input_table <- file.path(wd_fun, "metabolic/Metabolic_Sankey_diagram/Metabolic_Sankey_diagram_sur_input_rare.txt")
sub_input_table <- file.path(wd_fun, "metabolic/Metabolic_Sankey_diagram/Metabolic_Sankey_diagram_sub_input_rare.txt")
pl_input_table <- file.path(wd_fun, "metabolic/Metabolic_Sankey_diagram/Metabolic_Sankey_diagram_pl_input_rare.txt")
```
...
table_sub <- read.table(sub_input_table, header = T, sep = "\t")
colnames(table_sub) <- c("Taxa", "Reaction", "Freq")
table_pl <- read.table(pl_input_table, header = T, sep = "\t")
...
links_datset <- mutate(links,
                    group = case_when(
                      source == taxa_names[1] | target == taxa_names[1]  ~ 'a',
                      source == taxa_names[2] | target == taxa_names[2]  ~ 'b',
                      source == taxa_names[3] | target == taxa_names[3]  ~ 'c',
                      source == taxa_names[4] | target == taxa_names[4]  ~ 'd',
                      source == taxa_names[5] | target == taxa_names[5]  ~ 'e',
                      source == taxa_names[6] | target == taxa_names[6]  ~ 'f',
                      source == taxa_names[7] | target == taxa_names[7]  ~ 'g',
                      source == taxa_names[8] | target == taxa_names[8]  ~ 'h',
                      source == taxa_names[9] | target == taxa_names[9]  ~ 'i',
                      source == taxa_names[10] | target == taxa_names[10]  ~ 'j',
                      source == taxa_names[11] | target == taxa_names[11]  ~ 'k',
                      source == taxa_names[12] | target == taxa_names[12]  ~ 'l',
                      source == taxa_names[13] | target == taxa_names[13]  ~ 'm',
                      source == taxa_names[14] | target == taxa_names[14]  ~ 'n',
                      source == taxa_names[15] | target == taxa_names[15]  ~ 'o',
                      source == taxa_names[16] | target == taxa_names[16]  ~ 'p',
                      source == taxa_names[17] | target == taxa_names[17]  ~ 'q',
                      source == taxa_names[18] | target == taxa_names[18]  ~ 'r',
                      source == taxa_names[19] | target == taxa_names[19]  ~ 's',
                      source == taxa_names[20] | target == taxa_names[20]  ~ 't',
                      source == taxa_names[21] | target == taxa_names[21]  ~ 'u',
                      source == taxa_names[22] | target == taxa_names[22]  ~ 'v',
                    ))
...
my_color <- 'd3.scaleOrdinal() .domain(["a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q", "r", "s", "t", "u", "v","Carbon", "Nitrogen", "Sulfur", "Others", "my_unique_group"]) .range(["#4970a0", "#f8696b", "#FCCDE5", "#6A3D9A", "#1F78B4", "#FDB462", "#80B1D3", "#BEBADA", "#c988d1", "#A6CEE3", "#8DD3C7", "#B2DF8A", "#33A02C", "#FB9A99", "#FFFFB3", "#FDBF6F", "#E31A1C", "#d5416a","#FB8072", "#d87a71","#6a75d5", "#76c2d7", "#d87a71", "#FF7F00", "#836834", "#A6CEE3", "#c0c0c0"])'

# Make the Network. I call my colour scale with the colourScale argument
p <- sankeyNetwork(Links = links_datset, Nodes = nodes, 
