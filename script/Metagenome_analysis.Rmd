...
row_names <- MAG_abund_tab$`Genomic bins`
...
num.mags.plot <- dat3 %>% select(c(1, 2, 4)) %>%
  group_by(Category, Function) %>% 
  summarise(across(everything(), sum)) %>%
  filter(Function %in% c(C_N_metapathway, other_metapathway)) %>% 
...
p <- sankeyNetwork(Links = links, Nodes = nodes, Source = "IDsource", Target = "IDtarget", 
                   ...
                   nodeWidth = 20, nodePadding = 10, height = 580, width = 450,
                   sinksRight = F)
# save the widget
# library(htmlwidgets)
...
#         paste0(sankey.plots.folder, "/sankey_plot12.pdf"), delay = 0.2)
p
```
...
my_graph_pl <- table_pl[, c(2,3,4,5)] %>% 
  graph_from_data_frame()
dge_density(my_graph_sur, loops = TRUE)
dge_density(my_graph_sub, loops = TRUE)
dge_density(my_graph_pl, loops = TRUE)
