...
library(Hmisc)
envs <- metadata[, c("AI", "MAT", "NDVI", "HF", "Eh", "Clay", "Silt", "Sand", "Moisture", "pH",  "NH4_N", "NO3_N", "DON",       "SOC", "DOC", "LCP1", "LCP2", "RCP")]
envsn <- data.matrix(envs)
clust <- Hmisc::varclus(envsn)
plot(clust)
```

Based on the variable clustering results, we retain Clay, Silt, AI, NDVI, HF, NH<sub>4</sub><sup>+</sup>-N, Moisture, NO<sub>3</sub><sup>-</sup>-N, pH, MAT, Eh, LCP1, and RCP for the partial mantel test. 

## Difference in environmental factors among layers
```{r, fig.align='center', fig.width=8, fig.height=6}
my_comparisons <- list( c("SUR", "SUB"), c("SUB", "PL"), c("SUR", "PL") )
env_vars <- c("Layer", "Clay", "Silt", "pH", "Eh",  "Moisture", "SOC", "DOC", "LCP1", "RCP", "NH4_N", "NO3_N", "DON")
...
sel_env <- c("MAT", "AI", "NDVI", "HF", "Moisture",	"pH",	"Eh", "Clay",	"Silt",	 
...
partial.mantel.fun <- function(phylo) {
  ...
  df <- NULL
  vars <- c("MAT", "AI", "NDVI", "HF", "Moisture",	"pH",	"Eh", "Clay",	"Silt",	 
            "NH4_N",	"NO3_N", "LCP1", "RCP")
  for (x in vars) {
