...
env_vars <- c("Layer", "Clay", "Silt", "pH", "Moisture", "SOC", "DOC", "LCP1", "LCP2", "RCP", "NH4_N", "NO3_N", "DON")
...
p <- meta_dat %>% as.tibble() %>% select("Layer", env_vars) %>%
  ...
  facet_wrap(vars(env), scales = "free", ncol = 4) +
...
sel_env <- c("MAT", "AI", "NDVI", "HF", "Moisture",	"pH",	"Clay",	"Silt",	
           "SOC", "DOC", "NH4_N",	"NO3_N",	"DON",	
           "LCP1",	"LCP2",	"RCP")
```
...
library(Hmisc)
envs <- metadata[, c("AI", "MAT", "NDVI", "HF", "Clay", "Silt", "Sand", "Moisture", "pH",  "NH4_N", "NO3_N", "DON",       "SOC", "DOC", "LCP1", "LCP2", "RCP")]
envsn <- data.matrix(envs)
clust <- Hmisc::varclus(envsn)
plot(clust)
```

Based on the variable clustering results, we retain Clay, Silt, AI, "NDVI", "HF", NH<sub>4</sub><sup>+</sup>-N, Moisture, NO<sub>3</sub><sup>-</sup>-N, pH, MAT, LCP1, and RCP for the partial mantel test. 
...
partial.mantel.fun <- function(phylo) {
  ...
  df <- NULL
  vars <- c("MAT", "AI", "NDVI", "HF", "Moisture",	"pH",	"Clay",	"Silt",	 
...
rep_str = c("NH4_N" = expression(paste("NH"[4]^"+", "-N")),
...
ggsave(file.path(save.dir, './figs/env_effect/partail_matel_plot_for_revision.pdf'), p, width = 8.9, height = 4.5, units = "in")
p
```
