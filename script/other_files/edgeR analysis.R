...
############################################################################################################
# 2. KEGG decoder for all genes among three layers
kegg_decoder_fun <- function(layer, df) {
  ko_count_table = df
  samples <- grep(layer, meta_dat$sample_id, value = T)
  kegg_decoder_df = data.frame(Sample = c(), KEGG = c())
  for (kegg_id in rownames(ko_count_table)) {
    for (sample in samples) {
      if (ko_count_table[kegg_id, sample] > 0) {
...
kegg_decoder_sur_df <- kegg_decoder_fun('SUR', ko_count_table)
kegg_decoder_sub_df <- kegg_decoder_fun('SUB', ko_count_table)
kegg_decoder_pl_df <- kegg_decoder_fun('PL', ko_count_table)
...
write.table(kegg_decoder_sur_df,
            file.path(save.dir, 'tables/kegg/kegg_decoder/KEGG_decoder_sur_df_in.tsv'), sep='\t',  col.names=FALSE, row.names=FALSE , quote=FALSE)
write.table(kegg_decoder_sub_df,
            file.path(save.dir, 'tables/kegg/kegg_decoder/KEGG_decoder_sub_df_in.tsv'), sep='\t',  col.names=FALSE, row.names=FALSE , quote=FALSE)
write.table(kegg_decoder_pl_df,
            file.path(save.dir, 'tables/kegg/kegg_decoder/KEGG_decoder_pl_df_in.tsv'), sep='\t',  col.names=FALSE, row.names=FALSE , quote=FALSE)

# in the Data folder using the CryoBiome-kegg-decoder env: 
# cd /mnt/e/permafrost/result/tables/kegg/kegg_decoder
# python /mnt/e/permafrost/script/KEGG-decoder.py -i KEGG_decoder_sur_df_in.tsv -o KEGG_decoder_sur_df_output.tsv
# python /mnt/e/permafrost/script/KEGG-decoder.py -i KEGG_decoder_sub_df_in.tsv -o KEGG_decoder_sub_df_output.tsv
# python /mnt/e/permafrost/script/KEGG-decoder.py -i KEGG_decoder_pl_df_in.tsv -o KEGG_decoder_pl_df_output.tsv
# python -m pip install xxxx
KEGG_decoder_sur <- read.table(file.path(save.dir, "tables/kegg/kegg_decoder/KEGG_decoder_sur_df_output.tsv"), 
                               sep = "\t", header = TRUE)
KEGG_decoder_sub <- read.table(file.path(save.dir, "tables/kegg/kegg_decoder/KEGG_decoder_sub_df_output.tsv"),
                               sep = "\t", header = TRUE)
KEGG_decoder_pl <- read.table(file.path(save.dir, "tables/kegg/kegg_decoder/KEGG_decoder_pl_df_output.tsv"),
                              sep = "\t", header = TRUE)
...
p
############################################################################################################
# 3. KEGG decoder for the variable genes determined by the edgeR analysis
