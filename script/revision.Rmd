...
sel_ko_fermen_MAGs_pl <- temp_file %>%
  ...
  filter(ID %in% grep("_PL_", ID, value = T)) %>%
  select(c("Enzyme_protein_encoded", "presence", "Phylum")) %>%
  group_by(Phylum, Enzyme_protein_encoded) %>%
  summarise(across(everything(), sum))
...
fermen_genes_barplot_pl <- ggplot(sel_ko_fermen_MAGs_pl, aes(fill = Phylum, y = presence, 
                                                       x = Enzyme_protein_encoded)) + 
...
fermen_genes_barplot_pl
```

```{r, fig.align='center', fig.height=8.9, fig.width=8.9}
sel_ko_fermen_MAGs_sub <- temp_file %>%
  right_join(sel_ko, by = "KO") %>%
  filter(Category == "Fermentation") %>%
  select(-c("Category", "Subcategory", "KO")) %>%
  mutate(across(where(is.numeric), replace_na, 0)) %>%
  ...
  pivot_longer(cols = -c("Enzyme_protein_encoded"),
               ...
               values_to = "presence") %>%
  left_join(tax_tab, by = 'ID') %>%
  filter(ID %in% grep("_SUB_", ID, value = T)) %>%
  select(c("Enzyme_protein_encoded", "presence", "Phylum")) %>%
  group_by(Phylum, Enzyme_protein_encoded) %>%
  summarise(across(everything(), sum))

fermen_genes_barplot_sub <- ggplot(sel_ko_fermen_MAGs_sub, aes(fill = Phylum, y = presence, 
                                                       x = Enzyme_protein_encoded)) + 
  geom_bar(position = "stack", stat = "identity") +
  scale_fill_manual(values = mycols) + 
  labs(x = 'Fermentation pathways', y = 'Number of genes') +
...
fermen_genes_barplot_sub
```
```{r, fig.align='center', fig.height=8.9, fig.width=8.9}
sel_ko_fermen_MAGs_sur <- temp_file %>%
  right_join(sel_ko, by = "KO") %>%
  filter(Category == "Fermentation") %>%
  select(-c("Category", "Subcategory", "KO")) %>%
  mutate(across(where(is.numeric), replace_na, 0)) %>%
  ...
  pivot_longer(cols = -c("Enzyme_protein_encoded"),
               ...
               values_to = "presence") %>%
  left_join(tax_tab, by = 'ID') %>%
  filter(ID %in% grep("_SUR_", ID, value = T)) %>%
  select(c("Enzyme_protein_encoded", "presence", "Phylum")) %>%
  group_by(Phylum, Enzyme_protein_encoded) %>%
  summarise(across(everything(), sum))

fermen_genes_barplot_sur <- ggplot(sel_ko_fermen_MAGs_sur, aes(fill = Phylum, y = presence, 
                                                       x = Enzyme_protein_encoded)) + 
  geom_bar(position = "stack", stat = "identity") +
  scale_fill_manual(values = mycols) + 
  labs(x = 'Fermentation pathways', y = 'Number of genes') +
