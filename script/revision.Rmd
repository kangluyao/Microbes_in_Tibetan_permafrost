...
ch4_process_names <- list(
  ...
  'Soluable.methane.monooxygenase' = "Soluable methane monooxygenase"
)
...
ch4_flux_plot <- ch4_df %>% 
  ...
  xlab('Gene abundance (TPM)') +
  ylab(expression(paste("CH"[4], " production", " (ng", " g"^-1, " d"^-1, ")"))) +
  theme_bw() +
  theme(axis.title = element_text(size = 8), 
        axis.text = element_text(size = 7, colour = 'black'), 
        panel.grid = element_blank(),
...
N_vars <- c("amoA", "amoB",  "amoC", "hao", 
            "gdh_K00261", "gdh_K00262", "gdh_K15371", "glsA", "ureA", "ureC", "nmo",
            "glnA")
N_vars <- colnames(N_df)[-1]
