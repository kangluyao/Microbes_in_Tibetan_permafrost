...
seq_df <- read.table(file = file.path(wd, 'revision/data/cleandata.txt'),
                           sep = '\t',  header = T, stringsAsFactors = F)
MAG_tab <- rbind(MAG_df %>% dplyr::select(Layer, completeness, contamination) %>%
  pivot_longer(cols = -c("Layer"), names_to = "Index", values_to = "Value"),
  seq_df %>% dplyr::select(Layer, Clean_data, Recovery_rate_MAGs) %>%
  pivot_longer(cols = -c("Layer"), names_to = "Index", values_to = "Value"))
...
lm(Depth ~ predict(model), data = FTC_df) %>% broom::glance()
...
FTC_plot <- ggplot(FTC_df, aes(x = Depth, y = Days)) + 
  ...
  geom_smooth(method = "nls", 
              ...
              method.args = list(start = c(A = 212.48612, B = -0.19778, C = 7.36805)),
              size = 1, se = F, linetype = 1,  colour = 'black') +
  xlab("Soil depth (cm)") + ylab("Diurnal freezing-thawing cycles (Days)") +
  theme_bw() +
  theme(panel.grid = element_blank(), 
...
```{r}
filelist_sub = list.files("E:/permafrost/revision/metabolic_resample/metabolic_results_sub", pattern = "\\.txt$", full.names = TRUE)
#assuming tab separated values with a header    
datalist_sub = lapply(filelist_sub, function(x)read.table(x, header = F, sep = "\t")) 
#assuming the same header/columns for all files
datafr_sub = do.call("rbind", datalist_sub)


filelist_pl = list.files("E:/permafrost/revision/metabolic_resample/metabolic_results_pl", pattern = "\\.txt$", full.names = TRUE)
#assuming tab separated values with a header    
datalist_pl = lapply(filelist_pl, function(x)read.table(x, header = F, sep = "\t")) 
#assuming the same header/columns for all files
datafr_pl = do.call("rbind", datalist_pl)

colnames(datafr_sub) <- c("Taxa", "Reaction", "Freq")
colnames(datafr_pl) <- c("Taxa", "Reaction", "Freq")

table_sub <- datafr_sub %>% group_by(Taxa, Reaction) %>%
  summarise(across(everything(), mean))

table_pl <- datafr_pl %>% group_by(Taxa, Reaction) %>%
  summarise(across(everything(), mean))
```
...
taxa_names <- table_all %>% dplyr::select(Taxa, Freq) %>%
  group_by(Taxa) %>%
...
P <- sankeyNetwork(Links = links, Nodes = nodes, Source = "IDsource", Target = "IDtarget", 
                   ...
                   nodeWidth = 30, nodePadding = 10, height = 1225, width = 1968,
                   sinksRight = F)
# save the widget
# recommend using htmlwidgets
library(htmlwidgets)
saveWidget(P, file = paste0(save.dir.revision, "/sankey_plot.html"))
# library(webshot)
# install phantom:
